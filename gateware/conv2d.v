	...
	if (reset) begin
		...
		case(state)
			...
			if (rptr<(KHEIGHT-1)*IWIDTH*ICHAN)
				rptr <= (k==ICHAN*KWIDTH-1) ? rptr+ICHAN*IWIDTH-ICHAN*KWIDTH : rptr+'d1; // need modulo rowbuf size
			else
				rptr <= (k==ICHAN*KWIDTH-1) ? rptr-ICHAN*IWIDTH*(KHEIGHT-1)-ICHAN*KWIDTH : rptr+'d1; // need modulo rowbuf size
			...
			if (rcol>(IWIDTH-KWIDTH)) begin
				rcol <= 'd0;
				rcnt <= rcnt+(STRIDE*IWIDTH-KWIDTH);
				if (rptr<(KHEIGHT*IWIDTH*ICHAN)-(STRIDE*IWIDTH-KWIDTH))
					rptr <= rptr+(STRIDE*IWIDTH-KWIDTH);
				else
					rptr <= rptr+(STRIDE*IWIDTH-KWIDTH)-(KHEIGHT*IWIDTH*ICHAN*STRIDE);
			end
			else begin
				rcol <= rcol+STRIDE;
