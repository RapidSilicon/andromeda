module andromeda_tb;
	...
	integer error_cnts;
	reg [31:0] slv_addr;
	task all_slv_rw(input [31:0] n_slaves, input [31:0] n_csrs ); begin
	    wb_out = 'b0;
	    error_cnts = 0;
	...
	endtask
  	// ---------------------------------------------------------------
    	//  WB WRITE TASK
	task wb_write(input reg [32-1:0] wb_adr,input reg [32-1:0] wb_dat);begin
		wb_ADR <= wb_adr;
		wb_DAT_MOSI <= wb_dat;
		wb_CYC <= 1'b1;
		wb_STB <= 1'b1;
	...
	// ---------------------------------------------------------------
	//  WB READ TASK
	task wb_read(input [31:0] wb_adr);begin
		wb_ADR <= wb_adr;
		wb_CYC <= 1'b1;
		wb_STB <= 1'b1;
		wb_SEL <= 4'hF;
