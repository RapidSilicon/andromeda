...
def generate_batch(args):
    ...
    for i in range(args.batch):
        for j in range(14*16):
            if np.random.randint(2):
                ...
                xy=np.random.normal(loc=grid[j],scale=scale,size=[2000,2])
    ...
    for i in range(args.batch):
        ...
        fx=(1-0.90)+np.random.random()*0.90
        fy=(1-0.90)+np.random.random()*0.90
    ...
    cv2.imshow('gallery', cv2.resize(d[0],dsize=(800,700),interpolation=cv2.INTER_LINEAR)) 
    cv2.waitKey(1)
    return (d,l)
...
#cv2.imshow('gallery', cv2.resize(d[0],dsize=(800,700),interpolation=cv2.INTER_AREA))
#cv2.imshow('gallery', d[0])
#v=np.concatenate([cv2.resize(x,dsize=(320,280),interpolation=cv2.INTER_AREA) for x in d],axis=1)
#cv2.imshow('gallery', v)
cv2.waitKey(0)
exit()
...
def generate_batch(args):
    ...
    for i in range(args.batch):
        for j in range(14*16):
            if np.random.randint(2):
                l[i,j]=1
                xy=np.random.normal(loc=grid[j],scale=10.0,size=[1000,2])
                color=np.random.randint(128,256,size=3).astype(np.uint8)
                #color=[200,200,200]
                #print(i,j,'grid',grid[j],'color',color,'xy',xy)
                for (x,y) in xy:
